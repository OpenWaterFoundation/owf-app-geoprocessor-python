...
Functional tests run the GeoProcessor software in the installed environment and
therefore test the internal code, environment, operational tools, and workflow
...
1. Run individual tests (useful when working on a specific workflow/test):
	1. Create a script to run individual test:
		...
		2. ![Linux](images/linux-32.png) Linux: **Need to create example**
...
2. Run auto-generated test suite (useful when testing all software features before software release):
	...
	plus some additional testing commands for reporting.
		...
		command file in a terminal window to  auto-generate the full GeoProcessor test suite: **Need to create script**
	2. Run the command file created in the previous step:
		...
		2. ![Linux](images/linux-32.png) Linux:  **Need to create script**
...
Example of `geoprocessor-tests-out.gp.txt` file:
...
![summary](dev-tasks-images/geoprocessor-tests.gp.summary.html.png)
