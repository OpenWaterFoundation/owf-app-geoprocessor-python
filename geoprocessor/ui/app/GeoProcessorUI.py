...
class GeoProcessorUI(QtWidgets.QMainWindow):  # , Ui_MainWindow):
    ...
    def __init__(self, geoprocessor, runtime_properties=None):
    ...
    def ui_action_help_software_system_information(self):
        ...
        """
        try:
            # TODO smalers 2018-07-29 Format this into a scrollable text area and add more relevant information,
            # such as version of important dependencies, QGIS version, etc.
            program_home = app_util.get_property('ProgramHome')
            program_resources_path = app_util.get_property('ProgramResourcesPath')
            nl = '\n'
            qt_util.info_message_box(
                'ProgramHome="' + program_home + '"' + nl +
                'ProgramResourcesPath="' + program_resources_path + '"' + nl + nl +
                'Python version sys.version="' + sys.version + nl +
                'Python sys.path="' + str(sys.path) + nl,
                "Software/System Information")
        except Exception as e:
            # Should not happen but does during initial development and UI code swallows exceptions so output to log
            logger = logging.getLogger(__name__)
            message = "Problem showing Software/System Information"
            logger.warning(message, e, exc_info=True)
