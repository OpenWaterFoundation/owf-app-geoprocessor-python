...
rem this causes the cmd prompt to print "Must construct a QGuiApplication first."
...
rem Must reset these variables to refer to Python36. 
rem These variable are originally set within the CALL %OSGEO4W_ROOT%\bin\o4w_env.bat call. 
rem For more information, refer to following stack exchange post: 
rem https://gis.stackexchange.com/questions/251851/using-pycharm-with-qgis-3
...
rem REF: https://github.com/OSGeo/homebrew-osgeo4mac/issues/197
SET QGIS_PREFIX_PATH=%QGIS%

rem Add QGIS to the PATH environmental variable so that all QGIS, GDAL, OGR, etc. programs are found
SET PATH=%PATH%;%QGIS%\bin

rem Add Qt5 library to the path
rem http://spatialgalaxy.net/2018/02/13/quick-guide-to-getting-started-with-pyqgis3-on-windows/
...
rem set PYTHONPATH=%PYTHONPATH%;%OSGEO4W_ROOT%\apps\Python27\Lib\site-packages
...
rem set QGIS_PYTHON_EXE=%OSGEO4W_ROOT%\bin\python.exe
set QGIS_PYTHON_EXE=%OSGEO4W_ROOT%\bin\python3.exe
