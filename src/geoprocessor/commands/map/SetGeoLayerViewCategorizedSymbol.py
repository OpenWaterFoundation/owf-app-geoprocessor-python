...
class SetGeoLayerViewCategorizedSymbol(AbstractCommand):
    """
    ...
    __command_metadata['Description'] = "Set a GeoLayerView to use a categorized symbol."\
        "  Features will be drawn with symbol for each category value.\n"\
        "    GeoMapProject\n"\
        "      GeoMap [ ]\n"\
        "        GeoLayerViewGroup [ ]\n"\
        "          GeoLayerView [ ]\n"\
                        ...
                        if pv_GeoLayerViewID is None or pv_GeoLayerViewID == "":
                            ...
                            if len(geolayerviewgroup.geolayerviews) == 0:
                                ...
                                message = "No GeoLayerViews have been created for the layer view group. " \
                                          "Cannot determine default GeoLayerView."
                                recommendation = "Check that the GeoLayerViewID is valid."
                                self.logger.warning(message, exc_info=True)
                                self.command_status.add_to_log(CommandPhaseType.RUN,
                                                               CommandLogRecord(CommandStatusType.FAILURE, message,
                            ...
                            geolayerview_id = pv_GeoLayerViewID

                        geolayerview = geolayerviewgroup.get_geolayerview(geolayerview_id)
                        if geolayerview is None:
                            self.warning_count += 1
                            message = "GeoLayerView for GeoLayerView ID '{}' was not found.".format(
                                geolayerview_id)
                            recommendation = "Check that the GeoLayerViewID is valid."
                            self.logger.warning(message, exc_info=True)
                            self.command_status.add_to_log(CommandPhaseType.RUN,
