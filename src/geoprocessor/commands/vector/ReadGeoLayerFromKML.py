...
class ReadGeoLayerFromKML(AbstractCommand):
    """
    ...
    __command_parameter_metadata: [CommandParameterMetadata] = [
        ...
        CommandParameterMetadata("InputLayerName", str),
    ...
    __command_metadata['Description'] = (
        ...
        "If the KML file contains more than one layer (see <Folder><name>), specify the layer name to read."
    )
    ...
    # InputFile
    __parameter_input_metadata['InputFile.Description'] = "KML file to read"
    __parameter_input_metadata['InputFile.Label'] = "KML file"
    __parameter_input_metadata['InputFile.Required'] = True
    __parameter_input_metadata['InputFile.Tooltip'] = (
        "The KML file to read (relative or absolute path). ${Property} syntax is recognized.")
    __parameter_input_metadata['InputFile.FileSelector.Type'] = "Read"
    __parameter_input_metadata['InputFile.FileSelector.Title'] = "Select a KML file to read"
    __parameter_input_metadata['InputFile.FileSelector.Filters'] =\
        ["KML file (*.kml)", "All files (*.*)"]
    # InputLayerName
    __parameter_input_metadata['InputLayerName.Description'] = "layer name to read"
    __parameter_input_metadata['InputLayerName.Label'] = "Layer name"
    __parameter_input_metadata['InputLayerName.Required'] = False
    __parameter_input_metadata['InputLayerName.Tooltip'] = \
        "The name field <name> value in the file, to specify the layer to read.  Can use ${Property}."
    # GeoLayerID
    ...
    def check_runtime_data(self, input_file_absolute: str, input_is_url: bool, geolayer_id: str) -> bool:
        ...
        * the InputFile (absolute) uppercase ends in .KML (warning, not error)
        * the ID of the output GeoLayer is unique (not an existing GeoLayer ID)
        ...
        if input_is_url:
            ...
            if not os.path.isfile(input_file_absolute):
                ...
                if not input_file_absolute.upper().endswith(".KML"):
                    self.warning_count += 1
    ...
    def run_command(self) -> None:
        ...
        # noinspection PyPep8Naming
        pv_InputLayerName = self.get_parameter_value("InputLayerName")
        # noinspection PyPep8Naming
        pv_GeoLayerID =\
        ...
        # noinspection PyPep8Naming
        pv_InputLayerName = self.command_processor.expand_parameter_value(pv_InputLayerName, self)
        # noinspection PyPep8Naming
        ...
        if self.check_runtime_data(input_file_absolute, input_is_url, pv_GeoLayerID):
            ...
            try:
                # Create a QGSVectorLayer object with the KML InputFile.
                ...
                new_geolayer0 = VectorGeoLayer(geolayer_id=temp_geolayer_id,
                                               name=pv_Name,
                                               description=pv_Description,
                                               qgs_vector_layer=qgs_vector_layer,
                                               input_format=VectorFormatType.KML,
                ...
                # Mabye the internal memory management is different because KML file is difficult to output once read?
                #
                # To solve this problem, copy the initially read layer to a new instance and then add the copy to
                # the geoprocessor.
            ...
            # Raise an exception if an unexpected error occurs during the process.
                ...
                self.warning_count += 1
                message = "Unexpected error reading GeoLayer {} from KML file {}.".format(pv_GeoLayerID, pv_InputFile)
