...
def expand_formatter(absolute_path: str, formatter: str) -> str or None:
    """
    Returns the appropriate value of a parsed absolute path given the user-defined formatter code.
    Many of times the user will want the filename without the extension or just the extension from an absolute path.
    This function will return the desired parsed path given the user's input.
    ...
    # The %F formatter code returns the filename with the extension and without the leading path.
    # Print warning messages if the filename or extension are non-existent.
    # (The returned value, in those cases, will not be None but will instead be an empty string).
    if formatter == '%F':
    ...
    # The %f formatter code returns the filename without the leading path and without the extension.
    # Print warning messages if the filename is non-existent.
    # (The returned value, in that case, will not be None but will instead be an empty string).
    elif formatter == '%f':
    ...
    # The %E formatter code returns the extension with the '.' character.
    # Print warning messages if the extension is non-existent.
    # (The returned value, in that case, will not be None but will instead be an empty string).
    elif formatter == '%E':
...
def get_col_names_from_delimited_file(delimited_file_abs: str, delimiter: str) -> [str]:
    ...
    Returns:
        A list of strings. Each string represents a column name.
        If an error occurs within the function, None is returned.
    """
    ...
    try:
        ...
        with open(delimited_file_abs) as in_file:
            for lineNum, line in enumerate(in_file):
                ...
                # Return the column names of the header line in the delimited file.
                # The column names are items of a list and the column names are stripped of whitespaces on either side.
                if lineNum == 0:
...
def get_filename(full_path: str, remove_extension: bool = True) -> str:
    ...
    Args:
        full_path (str): the input full path
        remove_extension (bool): whether to remove the file extension
...
def print_standard_file_header(ofp: TextIO, comment_line_prefix: str = '#', max_width: int = 120,
    ...
    # Flush the file to make sure it is visible, such as if written to a long-writing file.
    ofp.flush()
...
def read_file(path: str, comment: str = None, return_comments: bool = True) -> [str]:
    ...
    Args:
        ...
        return_comments: Whether to return comment lines.
...
def to_relative_path(root_path: str, rel_path: str) -> str:
    ...
    For example, a "dir" of /a/b/c/d/e and a "path" of /a/b/c/x/y will result in ../../x/y.

    The strings passed in to this method should not end with a file separator
    (either "\" or "/", depending on the system).
    If they have a file separator, the separator will be trimmed off the end.
    ...
    This function will do error checking to make sure the directories passed in to it are not null or empty,
    but apart from that does no error-checking to validate proper directory naming structure.
    This function will fail with improper directory names (e.g., "C:\c:\\\\\\\\test\\\\").
    ...
    Returns:
        The relative path created from the two directory structures.
        This path will NOT have a trailing directory separator (\ or /).
        If both the root_path and rel_path are the same, for instance, the value "." will be returned.
        Plus the directory separator, this becomes ".\" or "./".

    Except:
        ValueError if the conversion cannot occur.  Most likely will occur
        in Windows when the two directories are on different drives.
    ...
    # If none of the above were triggered,
    # then the second folder is higher up the first directory's directory branch.

    # Get the final folder separator from the first folder,
    ...
    for i in range(start, -1, -1):  # Want to go to >= 0, hence -1 for second value
        ...
        if rel_path_to_check[0:(i+1)] == (root_path_to_check + '/')[0:(i+1)]:
            # A common start in the folder name has been found.
            # Count the number of separators in each folder to determine how much separation lies between the two.
            seps = {'\\', '/'}
