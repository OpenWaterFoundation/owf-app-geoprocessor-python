...
def get_os_type() -> str:
    """
    Determine the major operating system type, useful for showing system information but not advised for checks
    (use the is_*_os() functions to control program logic).
...
def is_cygwin_os() -> bool:
    ...
    Cygwin is a Linux OS but sometimes Cygwin-specific logic must be implemented.
...
def is_mingw_os() -> bool:
    ...
    MinGW is a Linux OS but sometimes MinGW-specific logic must be implemented.
...
def run_default_app(file_path: str, as_text: bool = False) -> None:
    ...
    Args:
        ...
        as_text (bool): Indicate whether the file should be treated as text,
            useful if the file extension is not associated with an application.
