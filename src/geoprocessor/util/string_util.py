...
def delimited_string_to_dictionary_one_value(delimited_string: str, entry_delimiter: str = ";",
    ...
    will result in a list {'key1': 'value1', 'key2': 'value2'}.
...
def delimited_string_to_dictionary_list_value(delimited_string: str, entry_delimiter: str = ";",
    ...
    will result in a list {'key1': ['value1', 'value1b', 'value1c'], 'key2': ['value2']}.
...
def key_value_pair_list_to_dictionary(key_value_list: [str]) -> dict:
    ...
    for key_value in key_value_list:
        # Split by equal sign.
        pos_eq = key_value.find("=")
        dict_key = ""
        dict_value = None
        if pos_eq > 1:
            # Get the key and value.
            dict_key = key_value[0:pos_eq].strip()
            dict_value = key_value[pos_eq + 1:].strip()
...
def parse_dictionary(dict_string: str) -> dict:
    """
     ...
     If value contains special characters (: and ,), it can be surrounded by single quotes to escape, for example:
          key1: 'value1', key2: 'value2'
     Args:
      dictString the dictionary string

     Returns:
        A dictionary containing the parsed values.  Any quoted values will have quotes removed.
     """
    dictionary = {}
    debug = False
    logger = None
    ...
    if (dict_string is not None) and (len(dict_string) > 0) and (dict_string.find(":") > 0):
        if dict_string.find("'") < 0:
            # Legacy code that works when no surrounding quotes.
            # First break map pairs by comma.
            pairs = dict_string.split(",")
            # Now break pairs and put in a dictionary.
            ...
            value = ""
            while pos < pos_last:
                pos = pos + 1  # Increment character.
                c = dict_string[pos]
                if debug:
                    logger.debug("pos={} c={} in_key={} in_value={} quoted={} keyLength={} key={} value={}.".format(
                                 pos, c, in_key, in_value, quoted, len(key), key, value))
                # Loop through the dictionary string.
                if not in_key and not in_value and (len(key) == 0):
                    if debug:
                    ...
                    if (c == '\'') and quoted:
                        ...
                        in_key = False
                        quoted = False
                    elif not quoted and c.isspace() or (c == ':'):
                        # Not quoted, found ending delimiter.
                        in_key = False
                    else:
                        # Add the character to the key.
                        key += c
                elif not in_key and not in_value and (len(key) > 0):
                    ...
                    if (c == '\'') and quoted:
                        ...
                        in_value = False
                        quoted = False
                        dictionary[str(key).strip()] = str(value).strip()
                        key = ""  # Clear.
                        in_key = False
                        in_value = False
                        quoted = False
                    elif not quoted and ((c == ',') or (pos == pos_last)):
                        # Not quoted, found ending delimiter.
                        in_value = False
                        dictionary[str(key).strip()] = str(value).strip()
                        key = ""  # Clear.
...
def parse_integer_list(s: str or None) -> [int]:
    """
    Parse a list of integers from string containing comma-separated list of individual integers and ranges,
    for example '1, 3-6, 17'.  The list will by default not be sorted.
