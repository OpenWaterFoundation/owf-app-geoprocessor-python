...
rem
rem Windows batch file to run pytest unit tests in QGIS/GeoProcessor development environment
...
set pytestBatVersion=1.0.0
set pytestBatVersionDate=2019-02-27

rem Pytest options
rem -q can also be used for quiet mode but -ra seems to override
set pytestOpts=-ra
...
title GeoProcessor OSGeo4W QGIS development environment to run run-pytest.bat (don't run gp.bat or gpui.bat)
...
echo Running "%PYTHONHOME%\python" -m pytest %pytestOpts% %*
"%PYTHONHOME%\python" -m pytest %pytestOpts% %*
...
rem Set the window title to indicate how configured so as to avoid confusion
title GeoProcessor standalone QGIS development environment to run run-pytest.bat (don't run gp.bat or gpui.bat)
...
echo.
echo Usage:  run-pytest.bat [options]
echo.
echo Run the GeoProcessor pytest unit tests in the development environment on Windows.
echo This batch file sets up the development environment and calls pytest.
echo.
...
echo.
echo gpdev.bat version %pytestBatVersion% %pytestBatVersionDate%
echo.
exit /b 0
